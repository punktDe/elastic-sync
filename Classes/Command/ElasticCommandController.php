...
class ElasticCommandController extends CommandController
{
     ...
     * @throws \PunktDe\Elastic\Sync\Exception\SynchronizationException
     */
    public function syncCommand(string $preset, $yes = false): void
    {
        ...
        if (!$yes && $this->output->askConfirmation(sprintf('Would you really like to remove and replace the index <b>%s</b> (y/n)?', implode(',', array_map(static function ($index) { return $index['indexName'];}, $localConfiguration->getIndices()))), false) !== true) {
            $this->quit(1);
        }
